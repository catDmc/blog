---
title: js深入之4类常见内存泄漏及如何避免
date: 2019-02-20 16:42:47
categories:
- 前端
tags:
- js
...
如果代码中保存了表格某一个 ` 的引用。将来决定删除整个表格的时候，直觉认为 GC 会回收除了已保存的 ` 以外的其它节点。实际情况并非如此：此 ` 是表格的子节点，子元素与父元素是引用关系。由于代码**保留了 ` 的引用**，导致整个表格仍待在内存中。所以保存 DOM 元素引用的时候，要小心谨慎。
